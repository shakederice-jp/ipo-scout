...
function MarkdownReport({text,beginner=false}:{text:string;beginner?:boolean}) {
  // AIが生成時に実際の改行ではなぁE"\n" とぁE��斁E���Eのまま出力してしまぁE��ースへの保険
  const normalizedText = text.replace(/\\n/g, "\n").replace(/\n{3,}/g, "\n\n");

  if(beginner){
    // 見�Eし的な行（体言止め�E短斁E��かどぁE��の判宁E
    const isHeadingLikeLine = (s: string) =>
      (s.length <= 28 && !s.includes('、E)) ||
      (!s.endsWith('、E) && !s.endsWith('.') && !s.endsWith('�E�E) && !s.endsWith('�E�E) && s.length <= 40);

    // 行を「見�Eし／箁E��書き／本斁E��に刁E��し、本斁E��ぁE��は意味段落としてまとめて1ブロチE��にする
    ...
    for (const line of normalizedText.split('\n')) {
      if (line.trim() === '') continue; // 空行�E段落の刁E��目の目安としてのみ扱ぁE��表示上�E無視すめE
      ...
      // 通常の斁E�Eバッファに連結し、ある程度の長さになったら1つの意味段落として確定すめE
      buffer += line;
        ...
        {blocks.map((b,i)=>{
          ...
          if(b.type==="bullet") return (
            <div key={i} style={{display:"flex",gap:8,marginBottom:10,padding:"8px 10px",backgroundColor:LIGHT,borderRadius:8}}>
              <span style={{color:PRIMARY,flexShrink:0}}>✁E/span>
...
const ALL_AXIS_LABELS:Record<string,string>={
  float:"需給の軽ぁE,lockup:"ロチE��アチE�E",timing:"上場タイミング",
  valuation:"バリュエーション",vc_sell:"VC売圧",growth:"成長性",
  management:"経営陣",unit_econ:"ユニットエコノミクス",competitor:"競合環墁E,
};
const ALL_AXIS_SHORT:Record<string,string>={
  float:"需給",lockup:"ロチE��アチE�E",timing:"タイミング",
  valuation:"バリュエ",vc_sell:"VC売圧",growth:"成長性",
  management:"経営陣",unit_econ:"収益性",competitor:"競吁E,
};
...
function parseAxisReport(text:string):{sections:Record<string,string>;positives:string[];negatives:string[];summary:string} {
  ...
  const extractBullets=(key:string)=>(sections[key]||"")
    ...
    .filter(Boolean);
  const positives=extractBullets("ポジチE��ブ要因");
  const negatives=extractBullets("ネガチE��ブ要因・リスク");
  const sugg=(sections["まとめE]||sections["投賁E��への示唁E]||sections["なぜ重要か"]||"").replace(/\*\*([^*]+)\*\*/g,"$1").trim();
...
function parseScenarioRange(vsIpo:string):[number,number] {
  ...
  const bai=vsIpo.match(/(\d+(\.\d+)?)\s*倁E);
...
function ScenarioCompareChart({scenarios,periodLabel,isLong}:{scenarios:Scenario[];periodLabel?:string;isLong?:boolean}) {
  ...
  const colorFor=(v:string)=>v==="強氁E?"#22c55e":v==="弱氁E?"#f87171":"#f59e0b";
  const textColorFor=(v:string)=>v==="強氁E?"#15803d":v==="弱氁E?"#b91c1c":"#92400e";
  ...
  const getPoints=(lo:number,hi:number,verdict:string)=>{
    ...
    const earlyMidOffset=verdict==="強氁E?15:verdict==="弱氁E?-15:0;
  ...
  return (
    <div style={{width:"100%",overflowX:"auto",marginBottom:14}}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",height:H,minWidth:480}}>
        {ticks.map(v=>(
          <g key={v}>
            ...
            <text x={x0-6} y={yFor(v)+4} textAnchor="end" fontSize={8}
              ...
              {(v/100).toFixed(v%100===0&&rawRange>60?1:2)}倁E
            </text>
          </g>
        ))}
        ...
        <text x={xMid} y={padT-6} textAnchor="middle" fontSize={8} fill="#94a3b8">上場直征E/text>
        {[...rows].reverse().map(r=>{
        ...
        <text x={x1} y={padT+chartH+14} textAnchor="middle" fontSize={9} fill="#64748b">{periodLabel||"期間終亁E}</text>
        {rows.map(r=>{
          ...
          return (
            <g key={r.id+"-label"}>
              ...
              <text x={x1+10} y={midY+5} fontSize={9} fill="#475569">
                {(r.lo/100).toFixed(2)}〜{(r.hi/100).toFixed(2)}倁E
              </text>
              <text x={x1+10} y={midY+17} fontSize={8} fill="#94a3b8">確玁Er.prob}</text>
            </g>
          );
        })}
      </svg>
      <p style={{fontSize:9,color:"#cbd5e1",marginTop:2}}>
        ※ 上場直後�E急激な価格変動を老E�Eした目安レンジです！EI試算�E中忁E��に対し前征E0%を仮定！E
      </p>
    </div>
  );
}
  ...
  return (
    <div style={{marginTop:10,padding:"16px 10px 8px"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
        ...
        <div style={{fontSize:9,fontWeight:700,color:"#ef4444",textAlign:"right",maxWidth:"60%",lineHeight:1.4}}>🔓 解除�E�{lockupPeriod}</div>
      </div>
    </div>
  );
}
      ...
      {open&&(
        <div style={{backgroundColor:"white",borderTop:`1px solid ${BORDER}`,padding:isBeginner?"14px":"10px 12px"}}>
          {isBeginner&&!beginnerDetail&&(
            <p style={{fontSize:11,color:"#92400e",backgroundColor:"#fffbeb",padding:"8px 10px",borderRadius:8,marginBottom:10}}>📖 初忁E��E��け解説は準備中のため、E��常の解説を表示してぁE��す、E/p>
          )}
        ...
        <div style={{borderTop:`1px solid ${BORDER}`,padding:"12px 16px 16px"}}>
          {isBeginner&&!hasBeginnerReport?(
            <div style={{backgroundColor:"#fffbeb",borderRadius:10,padding:"14px",border:"1px solid #fde68a",textAlign:"center"}}>
              <p style={{fontSize:12,color:"#92400e",lineHeight:1.8,margin:0}}>📖 こ�E頁E��の初忁E��E��け解説は、現在準備中です、Ebr/>もう少しお征E��ください、E/p>
            </div>
          ):isBeginner&&hasBeginnerReport?(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{backgroundColor:"white",borderRadius:10,padding:"10px 12px",border:`1px solid ${BORDER}`}}>
                <div style={{fontWeight:900,fontSize:10,color:TTEXT,marginBottom:4}}>📋 詳細刁E��</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <div style={{backgroundColor:"#f0fdf4",borderRadius:10,padding:"8px 10px",border:"1px solid #bbf7d0"}}>
                  <div style={{fontWeight:900,fontSize:9,color:"#15803d",marginBottom:3}}>✁E総詁E/div>
                ...
                <div style={{backgroundColor:"#fffbeb",borderRadius:10,padding:"8px 10px",border:"1px solid #fde68a"}}>
                  <div style={{fontWeight:900,fontSize:9,color:"#92400e",marginBottom:3}}>📄 確認書顁E/div>
...
function ScenarioCard({s}:{s:Scenario}) {
  const [open,setOpen]=useState(false);
  const isUp=s.verdict==="強氁E,isDown=s.verdict==="弱氁E;
        ...
        <div style={{backgroundColor:"white",borderTop:`1px solid ${vs.border}`,padding:"10px 12px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div>
              ...
              {(s.positives||[]).map((p,i)=><div key={i} style={{fontSize:10,color:"#475569",display:"flex",gap:4,marginBottom:2}}><span style={{color:"#22c55e",flexShrink:0}}>✁E/span>{p}</div>)}
            </div>
            <div>
              ...
              {(s.negatives||[]).map((n,i)=><div key={i} style={{fontSize:10,color:"#475569",display:"flex",gap:4,marginBottom:2}}><span style={{color:"#f87171",flexShrink:0}}>✁E/span>{n}</div>)}
            </div>
          </div>
          {s.conclusion&&(
            <div style={{backgroundColor:vs.bg,borderRadius:8,padding:"8px 10px",border:`1px solid ${vs.border}`,fontSize:10,color:"#475569",lineHeight:1.6}}>
              <span style={{fontWeight:900,color:vs.text}}><Info size={8} style={{display:"inline",marginRight:3}}/>要点�E�E/span>{s.conclusion}
            </div>
          )}
        </div>
      )}
    ...
    <div style={{position:"fixed",inset:0,backgroundColor:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{backgroundColor:"white",borderRadius:16,padding:24,maxWidth:360,width:"100%",border:`2px solid ${PRIMARY}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
          <div style={{fontWeight:900,fontSize:16,color:DARK}}>🔔 通知設宁E/div>
          <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,color:"#94a3b8"}}>✁E/button>
        </div>
        ...
        {status==="needsPlan"&&(
          <div style={{backgroundColor:"#fffbeb",border:"1px solid #fde68a",borderRadius:10,padding:12,marginBottom:12,fontSize:12,color:"#92400e"}}>
            {!userId
              ? <>通知機�Eを使ぁE��は<strong>ログイン</strong>が忁E��です、Ea href="/auth" style={{display:"block",marginTop:6,color:PRIMARY,fontWeight:700}}>ログイン・新規登録 ↁE/a></>
              : <>通知機�Eは<strong>通知プラン�E�¥890/月！E/strong>以上でご利用ぁE��だけます、Ea href="/" style={{display:"block",marginTop:6,color:PRIMARY,fontWeight:700}}>プランを確認すめEↁE/a></>
            }
          </div>
        )}
        {status==="done"&&(
          <div style={{backgroundColor:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,padding:12,marginBottom:12,fontSize:12,color:"#15803d"}}>
            ✁E通知設定を保存しました�E�毎週金曜18時にお知らせします、E
          </div>
        )}
        {status!=="needsPlan"&&status!=="done"&&(
          <>
            <div style={{fontSize:11,color:"#64748b",marginBottom:12}}>通知を受け取るイベントを選択してください�E�前週金曜18時送信�E�E/div>
            {([["notify_listing","🔴 上場日"],["notify_bb","🟦 BB開始日"],["notify_lockup_90","🔓 ロチE��アチE�E90日解除"],["notify_lockup_180","🔓 ロチE��アチE�E180日解除"]] as [keyof typeof settings,string][]).map(([k,label])=>(
...
/* 参老E��E��セクションの見�Eし（趁E��期�E短期�E長期�E3刁E��、実践皁E��則ペ�Eジと世界観を統一�E�E*/
/* 「詳細刁E�� 深掘りレポ�Eト」�EチE��ーと同じPRIMARYグリーンで統一し、アイコン背景だけをアクセントカラーで差し替える */
function ReferenceGroupHeader({icon,order,title,subtitle,accent}:{icon:string;order:string;title:string;subtitle:string;accent:string}) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:12,margin:"28px 0 4px",padding:"14px 16px",
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontWeight:900,fontSize:11,color:accent,letterSpacing:"0.02em",marginBottom:2}}>【参老E��E��{order}、E/div>
...
export default function AnalysisClient({company,initialAnalysis,visualizationData,allCompanies,hasAccess=true,level="expert"}:{company:IpoCompany;initialAnalysis:Analysis|null;visualizationData?:any;allCompanies?:any[];hasAccess?:boolean;level?:"expert"|"beginner"}) {
  ...
  if(!analysis) return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",backgroundColor:"#eef9f9"}}>
      <div style={{textAlign:"center",padding:24}}>
        ...
        <p style={{fontWeight:700,color:"#475569"}}>刁E��チE�Eタを取得できませんでした</p>
        <a href="/calendar" style={{display:"inline-block",marginTop:16,padding:"8px 16px",borderRadius:10,
          backgroundColor:PRIMARY,color:"white",fontWeight:700,fontSize:13,textDecoration:"none"}}>ↁEカレンダーへ戻めE/a>
      </div>
    </div>
  );
  ...
  const GROUPS=[
    {key:"ultra_short" as const,label:"趁E��朁E,sub:"初値売り�E当日トレーチE,icon:"⚡",color:"#ef4444",bg:"#fee2e2",anchor:"ultra"},
    {key:"short"       as const,label:"短朁E,  sub:"数週間〜数ヶ朁E,        icon:"📈",color:"#d97706",bg:"#fef3c7",anchor:"short"},
    {key:"long"        as const,label:"長朁E,  sub:"数年、E,                icon:"🏛",color:"#7c3aed",bg:"#ede9fe",anchor:"long"},
  ];

  const wrap:React.CSSProperties={maxWidth:720,margin:"0 auto",padding:"0 16px"};

  // 需給・VC刁E���E�趁E��期ブロチE��用�E��E中身を関数匁E
  const renderSupplyDemand=()=>{
    const sd=(company as any).structured_data;
    const lockupPeriod=sd?.ipo_details?.lockup_period||"上場征E80日";
    const floatRatio=sd?.ipo_details?.float_ratio||"参老E��";
    ...
    const chart=valid.length>0
      ?valid.slice(0,4).map((s:any,i:number)=>({label:s.name||`株主${i+1}`,pct:parseFloat(String(s.ratio||'0').replace('%','')),color:colors[i],lockup:s.lockup==="朁E?"ロチE��アチE�Eあり":"上場時より流E��E}))
      :[{label:"創業老E�E役員",pct:50,color:"#f87171",lockup:lockupPeriod},{label:"そ�E他株主",pct:35,color:"#fb923c",lockup:"吁E��条件あり"},{label:"一般投賁E���E��E募！E,pct:15,color:"#4ade80",lockup:"上場時より流E��E}];
    ...
    return(
      <Card>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
          ...
          <span style={{fontWeight:900,fontSize:14,color:"#1e293b"}}>需給・VC刁E��</span>
          <span style={{fontSize:9,color:"#94a3b8",backgroundColor:"#f1f5f9",padding:"2px 6px",borderRadius:10}}>参老E��</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:10}}>
          <svg width={sz} height={sz} style={{flexShrink:0}}>
            ...
            <text x={cx} y={cy-6} textAnchor="middle" fontSize={9} fill="#64748b">株主構�E</text>
            <text x={cx} y={cy+8} textAnchor="middle" fontSize={9} fill="#64748b">{valid.length>0?"(実データ)":"(概箁E"}</text>
          </svg>
          <div style={{flex:1,display:"flex",flexDirection:"column",gap:6}}>
            {chart.map((d:any,i:number)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:6}}>
                ...
                <span style={{fontSize:11,fontWeight:900,color:"#1e293b"}}>{valid.length>0?`${d.pct}%`:"目論見書参�E"}</span>
              </div>
            ))}
            {valid.length===0&&shareholders.length>0&&(
              ...
              主要株主�E�{(Array.isArray(shareholders)?shareholders:[]).slice(0,3).map((s:any)=>s.name).filter(Boolean).join('、E)}
              </div>
            )}
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
          {[{label:"流E��比率",val:floatRatio,c:"#d97706"},{label:"ロチE��アチE�E",val:lockupPeriod.replace("上場征E,"").replace("閁E,""),c:"#ef4444"},{label:"売り圧劁E,val:valid.length>0&&parseFloat(String(valid[valid.length-1]?.ratio||'0').replace('%',''))<=20?"佁E:"参老E��",c:"#475569"}].map(({label,val,c})=>(
  ...
  // 競合他社 財務比輁E��長期ブロチE��用�E��E中身を関数匁E
  const renderCompetitorFinancials=()=>{
    ...
    const parseJpNum=(s:string|null|undefined)=>{
      ...
      const n=parseFloat(s.replace(/[△▲\-,△冁E��丁E��]/g,"").replace(/,/g,""));
    ...
    return (
      <Card>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12}}>
          ...
          <span style={{fontWeight:900,fontSize:14,color:DARK}}>競合他社 財務比輁E/span>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
            <thead>
              <tr style={{backgroundColor:LIGHT}}>
                <th style={{padding:"8px 10px",textAlign:"left",fontWeight:900,color:TTEXT,fontSize:10,borderBottom:`1px solid ${BORDER}`}}>企業吁E/th>
                <th style={{padding:"8px 10px",textAlign:"right",fontWeight:900,color:TTEXT,fontSize:10,borderBottom:`1px solid ${BORDER}`}}>売上髁E/th>
                <th style={{padding:"8px 10px",textAlign:"right",fontWeight:900,color:TTEXT,fontSize:10,borderBottom:`1px solid ${BORDER}`}}>経常利盁E/th>
                <th style={{padding:"8px 10px",textAlign:"right",fontWeight:900,color:TTEXT,fontSize:10,borderBottom:`1px solid ${BORDER}`}}>当期純利盁E/th>
            ...
            <tbody>
              <tr style={{borderBottom:`1px solid ${BORDER}`,backgroundColor:PRIMARY+"22"}}>
                <td style={{padding:"8px 10px",fontWeight:900,color:DARK}}>🎯 {company.name}�E�EPO銘柄�E�E/td>
                <td style={{padding:"8px 10px",textAlign:"right",fontWeight:700,color:"#1e293b"}}>{ownRevenue!=null?`${ownRevenue}儁E�E`:"-"}</td>
                <td style={{padding:"8px 10px",textAlign:"right",fontWeight:700,color:"#1e293b"}}>{ownProfit!=null?`${ownProfit}儁E�E`:"-"}</td>
                <td style={{padding:"8px 10px",textAlign:"right",fontWeight:700,color:"#1e293b"}}>{ownNetProfit!=null?`${ownNetProfit}儁E�E`:"-"}</td>
                ...
                <td style={{padding:"8px 10px",textAlign:"right",color:"#64748b",fontSize:10}}>{ownFiscalYear||"目論見書参�E"}</td>
              </tr>
              {valid.map((c:any,i:number)=>(
                <tr key={i} style={{borderBottom:`1px solid ${BORDER}`,backgroundColor:i%2===0?"white":LIGHT}}>
                  ...
                  <td style={{padding:"8px 10px",textAlign:"right",color:"#1e293b"}}>{c.revenue!=null?`${c.revenue}儁E�E`:" E}</td>
                  <td style={{padding:"8px 10px",textAlign:"right",color:c.operating_profit>=0?"#15803d":"#ef4444"}}>{c.operating_profit!=null?`${c.operating_profit}儁E�E`:" E}</td>
                  <td style={{padding:"8px 10px",textAlign:"right",color:c.net_profit>=0?"#15803d":"#ef4444"}}>{c.net_profit!=null?`${c.net_profit}儁E�E`:" E}</td>
                  <td style={{padding:"8px 10px",textAlign:"right",color:"#1e293b"}}>{c.per!=null?`${c.per}倍`:" E}</td>
                  <td style={{padding:"8px 10px",textAlign:"right",color:"#64748b",fontSize:10}}>{c.fiscal_year||" E}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p style={{fontSize:10,color:"#94a3b8",marginTop:8,lineHeight:1.6}}>※ EDINETの有価証券報告書から自動取得。単位：億冁E��百丁E�E未満刁E��捨て�E�E/p>
      </Card>
    );
  };
    ...
    <div style={{backgroundColor:"#eef9f9",minHeight:"100vh",fontFamily:"'Noto Sans JP',sans-serif"}}>
     <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 16px",backgroundColor:"#e8f4f5",borderBottom:"1px solid #d0e8ea",flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:8,flexShrink:0}}>
          <button onClick={()=>setShowNotify(true)} aria-label="通知設宁E
          ...
          <button
            ...
            onClick={() => {
              ...
              const text = `📊【IPO刁E��、E{company.name}�E�E{company.ticker ?? ""}�E�\nAI総合評価�E�E{grade}ランク ${score}点/100点\n\n目論見書をAIが解析した詳細レポ�Eト�Eこちら👇\n#IPO #新規上場 #IPO投資`;
      ...
      <div style={{...wrap,paddingTop:12,paddingBottom:40,display:"flex",flexDirection:"column",gap:12}}>
            ...
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                  ...
                  {company.biz_type&&(
                    <div style={{backgroundColor:"rgba(255,255,255,0.85)",borderRadius:8,padding:"6px 10px"}}>
                      <div style={{fontSize:9,color:MID,fontWeight:700}}>業慁E/div>
        ...
        {/* こ�Eレポ�Eト�E読み方�E�総論�E吁E���E参老E��E��とぁE��設計思想の案�E�E�E*/}
        <div style={{display:"flex",alignItems:"flex-start",gap:8,padding:"10px 14px",borderRadius:10,
          ...
          <p style={{fontSize:11,color:"#64748b",lineHeight:1.8,margin:0}}>
            <strong style={{color:TTEXT}}>こ�Eレポ�Eト�E読み方�E�E/strong>
            まず「まずここに注目�E�」と「株価シナリオ」で全体像をつかみ、続く「詳細刁E�� 深掘りレポ�Eト」で9軸を掘り下げます、E
            そ�E先�E<strong style={{color:"#ef4444"}}>⚡趁E��朁E/strong>・<strong style={{color:"#d97706"}}>📈短朁E/strong>・<strong style={{color:"#7c3aed"}}>🌱長朁E/strong>、E
            ご�E身の投賁E��タイルに合わせて参老E��E��を選んでお読みください�E��E類�E老E��方は
            <a href="/ipo-guide" style={{color:PRIMARY,fontWeight:700}}>実践皁E��則ペ�Eジ</a>で解説してぁE��す）、E
          </p>
        </div>
        ...
        <Card>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:6,marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              ...
              <span style={{fontWeight:900,fontSize:14,color:DARK}}>AI刁E��要紁E/span>
            </div>
            {(analysis as any).data_confidence && (
              <span style={{
                ...
                {(analysis as any).data_confidence === "high" ? "✁E実データ引用" :
                 (analysis as any).data_confidence === "medium" ? "⚠�E�E一部推定含む" : "❁EチE�Eタ不足"}
              </span>
            )}
          </div>
          {(()=>{
            ...
            return (
              <>
                {isBeginner&&!beginnerSummary&&(
                  <p style={{fontSize:11,color:"#92400e",backgroundColor:"#fffbeb",padding:"8px 10px",borderRadius:8,marginBottom:10}}>📖 初忁E��E��け要紁E�E準備中のため、E��常の要紁E��表示してぁE��す、E/p>
                )}
                {summaryText.split(/\n\n+/).map((para:string,i:number)=>(
          ...
          {(analysis as any).missing_data_points?.length > 0 && (
            <div style={{marginTop:10,padding:"10px 12px",backgroundColor:"#fffbeb",borderRadius:8,border:"1px solid #fde68a"}}>
              <div style={{fontSize:10,fontWeight:700,color:"#92400e",marginBottom:6}}>📭 目論見書に記載がなかった頁E��</div>
              ...
              <p style={{fontSize:10,color:"#92400e",marginTop:8,paddingTop:8,borderTop:"1px solid #fde68a",lineHeight:1.6}}>
                ※これら�E頁E��は目論見書に記載がなく判断材料に欠けるため、AIをもってしても�E析できてぁE��ぁE��、あらかじめご亁E��ください、E
              </p>
            </div>
          )}
          {(analysis as any).data_citations?.length > 0 && (
            <div style={{marginTop:10,padding:"10px 12px",backgroundColor:"#f8fafc",borderRadius:8,border:"1px solid #e2e8f0"}}>
              <div style={{fontSize:10,fontWeight:700,color:"#64748b",marginBottom:6}}>📄 引用チE�Eタ根拠</div>
        ...
        {!hasAccess && (
          <div style={{borderRadius:14,padding:"20px",backgroundColor:"#0d4f52",color:"white",textAlign:"center"}}>
            <div style={{fontSize:15,fontWeight:900,marginBottom:6}}>🔒 ここから先�E有料コンチE��チE��ぁE/div>
            <p style={{fontSize:12,color:"#a0d4d6",margin:"0 0 14px",lineHeight:1.7}}>
            軸別スコア・シナリオ刁E��・詳細インサイトをご覧ぁE��だくには、レポ�Eト無制限�Eラン以上へのご加入、また�Eこ�E銘柄の単品購入�E�¥500�E�が忁E��です、E
            </p>
        ...
        {!userId && (
          <div style={{borderRadius:12,padding:"14px 16px",backgroundColor:"#fffbeb",border:"1px solid #fde68a",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <div>
                <div style={{fontWeight:900,fontSize:13,color:"#082b2e"}}>BB開始�E上場日・公募価格確定をお知らせ</div>
                <div style={{fontSize:11,color:"#92400e",marginTop:2}}>通知プランで前週金曜18時に自動�E信</div>
              </div>
            </div>
        ...
        {insights.length>0&&(
          <Card>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
              ...
              <span style={{fontWeight:900,fontSize:14,color:"#1e293b"}}>まずここに注目�E�E/span>
            </div>
        ...
        {/* ⑫⑬ 株価シナリオ刁E���E�投賁E��ミュレーション */}
        <Card>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
            <BarChart2 size={14} color={PRIMARY}/>
            <span style={{fontWeight:900,fontSize:14,color:"#1e293b"}}>株価シナリオ刁E��</span>
          </div>
          <div style={{display:"flex",gap:6,marginBottom:10}}>
            {(["short","long"] as const).map(tab=>(
              <button key={tab} onClick={()=>setScenTab(tab)} style={{display:"flex",alignItems:"center",gap:4,padding:"6px 12px",borderRadius:8,fontSize:12,fontWeight:900,cursor:"pointer",border:"none",backgroundColor:scenTab===tab?DARK:"#f1f5f9",color:scenTab===tab?"white":"#64748b"}}>
                {tab==="short"?<><Clock size={10}/>短期（、Eヶ月！E/>:<><Calendar size={10}/>長期！E、E0年�E�E/>}
              </button>
            ))}
          </div>
          <ScenarioCompareChart scenarios={scenTab==="short"?scenarios_short:scenarios_long} periodLabel={scenTab==="short"?"6ヶ月征E:"5、E0年征E} isLong={scenTab==="long"}/>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {scenTab==="short"?(
  scenarios_short.length>0
    ?scenarios_short.map((s:any)=><ScenarioCard key={s.id} s={s}/>)
    :<div style={{textAlign:"center",padding:"24px",color:"#94a3b8",fontSize:13}}>シナリオ生�E中...</div>
):(
  ...
  <p style={{fontSize:11,color:"#0369a1",margin:0,lineHeight:1.7}}>
    吁E��ナリオの背景にある財務データ・需給構造・市場環墁E�E詳細は、Ebr/>
    <strong>下部の「詳細刁E�� 深掘りレポ�Eト、E/strong>に9軸で体系皁E��整琁E��てぁE��す、E
  </p>
</div>
{(()=>{
            const ipoPrice = (company as any).ipo_price;
            const scenarios = scenTab==="short" ? scenarios_short : scenarios_long;
            const periodLabel = scenTab==="short" ? "6ヶ月征E : "5、E0年征E;
            ...
            return (
              <div style={{marginTop:16,borderRadius:12,overflow:"hidden",border:"1px solid #d0f0f0"}}>
                <div style={{backgroundColor:"white",padding:"12px 16px",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid #d0f0f0"}}>
                  ...
                  <div>
                  <div style={{fontWeight:900,fontSize:14,color:"#082b2e"}}>投賁E��ミュレーション�E�E00株購入の場合！E/div>
                  <div style={{fontSize:10,color:"#6b9ea0"}}>公募価格が決定した場合�E試箁E/div>
                  </div>
                </div>
                ...
                {!ipoPrice ? (
                  <div style={{backgroundColor:"white",padding:"20px 16px",textAlign:"center"}}>
                    ...
                    <div style={{fontWeight:900,fontSize:14,color:"#082b2e",marginBottom:8}}>公募価格確定後に自動表示されまぁE/div>
                    <p style={{fontSize:12,color:"#64748b",lineHeight:1.8,marginBottom:16}}>
                      {company.name}�E�E(company as any).ticker ?? ""}�E��E公募価格はブックビルチE��ング期間終亁E��に決定されます、Ebr/>
                      価格決定後�E当サイトに再度ご訪問いただくと、�E体的な投賁E��額シミュレーションをご確認いただけます、E
                    </p>
                    <button
                  ...
                  <div style={{backgroundColor:"white",padding:"16px"}}>
                    <div style={{display:"flex",gap:12,marginBottom:16,padding:"10px 14px",backgroundColor:"#f0fafa",borderRadius:10}}>
                      ...
                      <div style={{textAlign:"center"}}>
                        <div style={{fontSize:10,color:"#6b9ea0"}}>最小投賁E��！E00株�E�E/div>
                        <div style={{fontWeight:900,fontSize:18,color:"#082b2e"}}>¥{(ipoPrice*100).toLocaleString()}</div>
                      </div>
                    </div>

                    <div style={{fontSize:11,fontWeight:700,color:"#0d4f52",marginBottom:8}}>{periodLabel}のシナリオ別試箁E/div>
                    <div style={{overflowX:"auto"}}>
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                        <thead>
                          <tr style={{backgroundColor:"#f0fafa"}}>
                            ...
                            <th style={{padding:"8px 10px",textAlign:"right",color:"#6b9ea0",fontWeight:700,borderBottom:"1px solid #d0f0f0"}}>評価顁E100株)</th>
                        ...
                        <tbody>
                          {scenarios.map((s:any)=>{
                            ...
                            if(s.vsIpo.includes("倁E)&&nums.length>=1){
                            ...
                            return (
                              <tr key={s.id} style={{backgroundColor:bg,borderBottom:"1px solid #f0fafa"}}>
                                <td style={{padding:"10px 10px",fontWeight:900,color:col}}>
                                  {icon} {s.verdict}<br/>
                                  <span style={{fontSize:9,fontWeight:400,color:"#94a3b8"}}>確玁Es.prob}</span>
                                </td>
                                ...
                                <td style={{padding:"10px 10px",textAlign:"right",fontWeight:900,color:loPnl>=0?"#15803d":"#b91c1c"}}>
                                  {loPnl!==0?(samePnl?pnlStr(loPnl):`${pnlStr(loPnl)}、E{pnlStr(hiPnl)}`):"計算中"}
                                </td>
                              </tr>
                            );
                          })}
                    ...
                    <div style={{marginTop:12,padding:"10px 12px",backgroundColor:"#fffbeb",borderRadius:8,border:"1px solid #fde68a"}}>
                      <p style={{fontSize:10,color:"#92400e",lineHeight:1.8,margin:0}}>
                        ⚠�E�E【重要】本シミュレーションは、IPO企業が��融庁に提�Eした目論見書をAIが�E析した結果に基づく試算値であり、実際の株価を保証するも�Eではありません。実際の株価は市場環墁E�E需給・業績等により大きく影響を受け、試算値より大幁E��乖離することがあります。想定外�E値動きが生じた場合�E、ご自身の判断で躊躁E��く損刁E��等�E対応をご検討ください。投賁E��断および結果に対する責任は当サービスでは負ぁE��ねます、E
                      </p>
                    </div>
                  </div>
                )}
          ...
          <div style={{backgroundColor:PRIMARY,padding:"16px 20px"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,marginBottom:12}}>
              <div>
                ...
                <h2 style={{fontWeight:900,fontSize:18,color:DARK,margin:"0 0 2px"}}>詳細刁E�� 深掘りレポ�EチE/h2>
                <p style={{fontSize:10,color:MID,margin:"0 0 8px"}}>投賁E��間軸�E�趁E��期�E短期�E長期）で整琁E��ぁE軸刁E��</p>
          ...
          <div style={{display:"flex",justifyContent:"center",padding:"8px 0 4px"}}>
            <a href="/ipo-guide" style={{fontSize:12,color:"#66c3c6",textDecoration:"none",fontWeight:700,display:"flex",alignItems:"center",gap:6,padding:"8px 20px",borderRadius:20,border:"1px solid #66c3c6",background:"#f0fdf4"}}>
              ...
              <span style={{display:"flex",flexDirection:"column",lineHeight:1.5}}>
                <span>IPO投賁E��賁E��を増やぁE/span>
                <span>実践皁E��則�E�趁E��期�E短期�E長期別�E�E/span>
              </span>
              <span>ↁE/span>
            </a>
          </div>
       ...
       {/* 事業等�Eリスク�E�重要度別�E�：深掘りレポ�Eト�E直後、参老E��E��の手前に配置 */}
       {visualizationData && <RiskTable vizData={visualizationData} />}

{/* ===== ここから参老E��E���E�趁E��期�E短期�E長期�E3刁E��！E===== */}

<ReferenceGroupHeader
  icon="⚡" order="①" title="趁E��期投賁E��向け" subtitle="初値で勝つ�E��E募条件・需給・類似IPOの着地点を確誁E
  accent="#ef4444"
/>
        ...
        <ReferenceGroupHeader
          icon="📈" order="②" title="短期投賁E��向け" subtitle="ロチE��アチE�Eを読む�E�総合スコア・価格の妥当性・今後�EイベンチE
          accent="#d97706"
        />
        <Card>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
            <BarChart2 size={14} color="#d97706"/>
            <span style={{fontWeight:900,fontSize:14,color:"#1e293b"}}>パフォーマンス・レーダー�E�E軸�E�E/span>
          </div>
        ...
        <ReferenceGroupHeader
          icon="🌱" order="③" title="長期投賁E��向け" subtitle="10倍株を狙ぁE��業績実績・株主構�E・賁E��使途�E事業リスク"
          accent="#7c3aed"
        />
        ...
        {(analysis.sources||[]).length>0&&(
          <Card>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
              ...
              <span style={{fontWeight:900,fontSize:13,color:"#475569"}}>参老E��献・確認�E</span>
            </div>
            {(analysis.sources||[]).map(src=>(
              <div key={src.url} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:4}}>
                <span style={{color:"#cbd5e1",fontSize:10,flexShrink:0,marginTop:1}}>ↁE/span>
...
{allCompanies && allCompanies.length > 1 && (()=>{
          ...
          return (
            <div style={{display:"flex",gap:8,justifyContent:"space-between"}}>
              <a href={prev?`/analysis/${prev.id}`:undefined as any}
                ...
                <div style={{fontSize:10,color:"#94a3b8",marginBottom:2}}>ↁE前�E銘柄</div>
              ...
              <a href={next?`/analysis/${next.id}`:undefined as any}
                ...
                <div style={{fontSize:10,color:"#94a3b8",marginBottom:2}}>次の銘柄 ↁE/div>
        ...
        <div style={{textAlign:"center",fontSize:10,color:"#94a3b8",paddingTop:8}}>
          <div style={{fontWeight:900,fontSize:11,color:"#64748b",marginBottom:4}}>⚠ 免責事頁E/div>
          <p style={{lineHeight:1.7,maxWidth:560,margin:"0 auto 8px"}}>
            本レポ�Eト�EAIによる惁E��整琁E�E判断材料の提供を目皁E��しており、E
            <strong style={{color:"#475569"}}>投賁E��誘ではありません、E/strong>
            スコア・価格目標�EAIの試算値であり封E��の結果を保証しません。最終的な投賁E��断はご�E身の責任において行ってください、E
          </p>
