...
async function callGemini(prompt: string, maxOutputTokens: number): Promise<{ text: string; truncated: boolean }> {
...
export async function generateWithGemini(prompt: string): Promise<string> {
  ...
  // 保存・表示されてしまっていた。
  let result = await callGemini(prompt, 3000);
  if (result.truncated) {
    console.warn("generateWithGemini: MAX_TOKENSで打ち切り検知、上限を引き上げて再試行します");
    result = await callGemini(prompt, 6000);
    if (result.truncated) {
