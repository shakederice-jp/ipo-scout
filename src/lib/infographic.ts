...
export interface InfographicData {
  ...
  radarScores?: { id: string; label: string; score: number | null }[]; // 2026/9/4追加: 9軸個別スコア(レーダーチャート用)。src/lib/ipo-axis-scores.tsのcomputeIndividualAxisScores()で作る。score=nullの軸は描画側で除外する
}
...
export async function createInfographic(data: InfographicData): Promise<string | null> {
  try {
    const params = new URLSearchParams({
      ...
      ...(data.radarScores && data.radarScores.some(r => r.score != null) ? { radarScores: JSON.stringify(data.radarScores) } : {}),
    });
