...
- 全体は「SDS法」(Summary要約→Details詳細→Summaryまとめ)の型で構成すること
  - 冒頭(Summary): 今後2週間で何社が上場予定か、件数・期間を1〜2文で端的に要約する
  - 中盤(Details): 銘柄ごとの詳細(会社概要や直近の業績、注目ポイントなど、事実として分かっている情報)を具体的に書く
  - 末尾(Summary): 全体を通して投資家が押さえておくべき点を1〜2文で簡潔にまとめ、そのまま分析レポートページへの誘導に続けること
...
# 記載のポイント
- 全体は「SDS法」(Summary要約→Details詳細→Summaryまとめ)の型で構成すること
  - 冒頭(Summary): 今週注目すべきイベントが何件あるか、期間とあわせて1〜2文で端的に要約する
  - 中盤(Details): イベントごとの詳細と、株式相場への影響を具体的に書く
  - 末尾(Summary): 今週全体を通じて特に押さえておきたい点を1〜2文で簡潔にまとめる
...
//
// 2026/9/23追記②: 「答え合わせ」記事は他のX投稿テーマ(共通STYLE_GUIDE=です/ます禁止・
// 体言止め・IR速報レポート風)とは別に、この記事専用の会話調スタイルを使う。読者と
// 投資仲間として会話しているような、問いかけを交えた親しみやすいトーンにするため。
// 他の7テーマ(IPOカレンダー・経済指標など)は従来通りSTYLE_GUIDEのまま変更しない。
const CHECKPOINT_STYLE_GUIDE = `
# 文体ルール(答え合わせ記事専用・厳守)
- 「です・ます」調の自然な話し言葉に近い文章で書く。体言止めやIR速報レポート風の言い回しは使わない
- ▼①②③のような見出し記号や箇条書きの多用はしない。読み物として段落でつなげる
- 「当分析室」を一人称に使いつつ、投資仲間に語りかけるような親しみやすいトーンで書く
- 結果に対する驚き・納得・意外性など、感情のこもった一言を自然に混ぜる(大げさな煽りは禁止)
- 全体で600〜900文字程度で、要点に絞って書く(詳細な箇条書きレポートにはしない)
- 絵文字は使わない、または使っても1つ程度に留める
- URLは含めない
`;
...
- 必ず最後の1〜2文は、読者に語りかける問いかけで締めること(例:「皆さんの予想は当たっていましたか?」「次のIPOでも一緒に答え合わせしていきましょう」のような、投資仲間に話しかけるトーン。ただし一字一句この例文通りでなくてよい)

${CHECKPOINT_STYLE_GUIDE}
