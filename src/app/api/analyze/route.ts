...
5. appeal_narrativeは、X投稿・マーケットトレンドページ用の「無料で読める客寄せ文」であり、bodyとは全く別の独立した文章として作成すること。**500〜650字程度**(2026/9/1、文章量が多すぎる・丁寧すぎるとの指摘を受けて600〜800字から短縮)。**この文章の目的は分析ページの内容を要約して伝えることではなく、分析ページを読みたいと思わせることである点に注意し、以下を満たすこと**:
   - 全体は3段落構成とし、セールスライティングの型「PASONA」を土台に、次の役割を段落ごとに持たせること(ラベル自体は出力に書かないこと):
     ①1段落目＝Problem(問題提起): この銘柄について投資家が気になるはずの具体的な事実・数字を1つ提示する
     ②2段落目＝Agitation(あおり): その事実が意味しうる問い・見過ごすと機会損失や判断ミスにつながりかねない点を、断定せず問いかけの形で強調する
     ③3段落目＝Solutionの手前で止める: 「その答え・判断材料は分析ページで詳しく扱っている」ことをそれとなく示唆しつつ、具体的な結論(Solution)そのものは書かない
   - 各段落は「具体的な数値・事実を1つ提示→その数値から浮かぶ疑問を1つだけ、簡潔に問いかける」というテンポの良い往復にすること。手本となる書き方の例:「わずか4年で売上を5.8億円から55.8億円へと約10倍に伸ばした急成長企業です。あなたなら、この成長スピードをどう評価するでしょうか。」のように、事実の直後に短い問いを1つ添えてすぐ次の話題に移り、1つの段落の中で問いかけや相づちを何度も重ねないこと
   ...
   - 開示してよい具体的な実データは、最もインパクトのある2〜3点(急成長率や赤字幅、株主構成等)に絞ること。財務指標を何点も列挙したり、複数の数値の根拠・背景・要因を詳しく説明したりしないこと(それは有料の分析ページの役割であり、ここで詳しく説明してしまうと分析ページを読む理由が無くなってしまう)
   ...
   - **「〜と感じた方も多いのではないでしょうか」「想像してみてください」のような、具体的な情報を含まない相づち・共感フレーズを分量稼ぎのために使わないこと。分量は詳しい説明や語りかけの言い回しを増やして確保するのではなく、簡潔な事実提示と端的な問いかけをテンポよく繰り返す構成で、無理に長くしないこと**
   - 断定的な投資助言(「買うべき」「今が買い時」等)や、実データに無い誇張・煽り文句は禁止。あくまで事実ベースで、書き方・問いかけでフックを作ること
   - 文字数は500〜650字を目安とし、この範囲に収まるよう最後まで書き切ってから終えること(文字数超過を避けるために文を切り詰めて終えるのではなく、全体の分量を調整すること)
...
【出力形式】必ず以下の構造のみで完結させること:
{
  ...
  "appeal_narrative": "500〜650字程度の客寄せ文。上記ルール5の通り、PASONA(Problem→Agitation→Solution手前で止める)の3段落構成で、事実提示→短い問いかけのテンポにまとめ、開示する具体的データは2〜3点に絞り、理由・背景・評価は問いかけのまま残し、文自体は完結させること。ですます調"
}`;
}
