...
5. appeal_narrativeは、X投稿・マーケットトレンドページ用の「無料で読める客寄せ文」であり、bodyとは全く別の独立した文章として作成すること。**500〜650字程度**(2026/9/1、文章量が多すぎる・丁寧すぎるとの指摘を受けて600〜800字から短縮)。**この文章の目的は分析ページの内容を要約して伝えることではなく、分析ページを読みたいと思わせることである点に注意し、以下を満たすこと**:
   - 基本構成は3段落程度。各段落は「具体的な数値・事実を1つ提示→その数値から浮かぶ疑問を1つだけ、簡潔に問いかける」というテンポの良い往復にすること。手本となる書き方の例:「わずか4年で売上を5.8億円から55.8億円へと約10倍に伸ばした急成長企業です。あなたなら、この成長スピードをどう評価するでしょうか。」のように、事実の直後に短い問いを1つ添えてすぐ次の話題に移り、1つの段落の中で問いかけや相づちを何度も重ねないこと
...
【出力形式】必ず以下の構造のみで完結させること:
{
  ...
  "appeal_narrative": "500〜650字程度の客寄せ文。上記ルール5の通り、事実提示→短い問いかけのテンポで3段落程度にまとめ、開示する具体的データは2〜3点に絞り、理由・背景・評価は問いかけのまま残し、文自体は完結させること。ですます調"
}`;
}
