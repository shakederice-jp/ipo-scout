...
export async function POST(req: NextRequest) {
  ...
  if (!chart_type || !CHART_TYPES.includes(chart_type)) {
    return NextResponse.json(
      { error: "chart_type は revenue_chart / shareholders_chart / valuation_table のいずれかを指定してください" },
      { status: 400 }
    );
  }
  ...
  const prompt = buildPrompt(chart_type as ChartType, (co as any).name, sd);
