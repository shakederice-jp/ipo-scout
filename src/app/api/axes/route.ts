...
// 2026/9/6追加: 6見出し全体を、ビジネス文書向けの説得構成「PREP法」
// (Point結論→Reason理由→Example具体例→Point結論)を土台に設計している。
// 「なぜ重要か」でPoint→Reasonを示し、「データ分析」がその裏付けとなるExample、
// 「まとめ」で最初のPointを言い換えて再提示することで、結論から入り結論で締める
// 一貫した流れを作る(読者が最後に何を持ち帰ればよいかが明確になる)。
const REPORT_SECTIONS: Record<number, string> = {
  1: `### なぜ重要か
この指標が{PERIOD}のIPO投資判断に与える影響を200〜350字程度で説明してください。1文目は、この指標についてのこの銘柄の結論(Point)を一言で言い切ってください。「〇〇という点で、この指標は注目に値します」のように、読み手がそれだけで要点を掴める断定的な着地の一文にすること。そのあと空行を1行挟んでから、なぜそう言えるのかという理由(Reason)の説明を続ける2段落構成にしてください。

### データ分析
目論見書の具体的数値・事実を引用しながら200〜350字程度で分析してください。この段落は「なぜ重要か」で述べた結論を裏付ける具体例(Example)としての役割を持たせ、意味のまとまりごとに空行を1行挟み、2〜3段落に分けてください（1段落を100字程度が目安）。
...
### まとめ
この軸から読み取れる材料を200〜350字程度でニュートラルに整理してください。1文目は、冒頭の「なぜ重要か」で述べた結論(Point)を、読み手により腹落ちする言葉で言い換えて再提示すること(結論→理由→具体例→結論、というPREP型の締めくくりとして機能させる)。そのあと空行を1行挟んでから補足の説明を続ける2段落構成にしてください。
...
・ロックアップの解除株価(公開価格の◯倍)、売出比率、代表者の持株比率、市場規模に対する売上の割合(浸透率)は、画面上で各軸の見出し横にバッジとして別途表示しています。本文でこれらの数値を改めて計算・列挙・繰り返さないでください(文章を長くしないため)。各セクションの指定文字数の範囲内で、上限寄りではなく簡潔にまとめてください。`;
